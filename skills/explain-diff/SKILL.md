...
- **`html` is the only field you write markup into. Every other field is plain text, and marks code with backticks.** So in an `html` field, an identifier is `<code>foo</code>` and a backtick renders as a literal backtick; in a `"title"`, a `"heading"`, the `"subtitle"` or a quiz `question`/`text`, it is `` `foo` `` and a `<code>` tag renders as visible angle brackets. Both halves have shipped broken pages.
- **Write the literal character, never the entity** — `Background & intuition`, not `Background &amp; intuition`. The renderer escapes these fields for you, so an entity you encode yourself reaches the reader as `&amp;amp;`.
- **A tag in a `"title"` or a `"heading"` is refused**: `render.py` exits 1, names the field and prints the backtick form to write instead, because a heading is the one place a tag can only be a mistake. Quiz text and the subtitle stay permissive — a question that quotes HTML is legitimate and renders as text.
