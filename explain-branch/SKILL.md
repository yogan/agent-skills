...
2. **One chapter per substantial commit**, in build order, `id: "chapter-N"`, heading titled
   by what the commit *does* (not its raw subject line) — e.g. "Chapter 2: Making `x-cu-method`
   mandatory", not "Chapter 2: d157b1d". Directly under the `<h2>`, add one small muted line
   citing the real commit for traceability:
   `<p style="color:var(--muted); margin-top:-.5rem; font-size:.85em;">commit <code>d157b1d</code> — refactor(PP3-285): make x-cu-method explicit, skip fields without one</p>`
